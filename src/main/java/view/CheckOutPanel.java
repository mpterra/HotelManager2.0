			...
			public void mouseClicked(MouseEvent evt) {
				if (evt.getClickCount() == 2) {
					...
					if (row != -1) {
						...
						EditCheckOutDialog dialog = new EditCheckOutDialog(
								SwingUtilities.getWindowAncestor(CheckOutPanel.this), idHospedagem);
	...
	private void atualizarTabela() {
		...
		try (Connection conn = DatabaseConnector.conectar();
			...
			while (rs.next()) {
				...
				modelo.addRow(new Object[] { rs.getInt("id"), rs.getString("hospede"), rs.getString("email"),
						rs.getString("telefone"), rs.getString("cama"), entradaFormatada, saidaFormatada });
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
